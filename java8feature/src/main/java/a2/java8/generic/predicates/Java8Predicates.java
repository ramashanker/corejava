...
public class Java8Predicates {
	public static void main(String[] args) {
		...
		Employee e=firstMatchingWithAny(employees, emp->emp.getName().equals("test2"));
		System.out.println(e.getEmpid() + ":" + e.getName() + ":" + e.getSalary());	
		Person p=firstMatchingWithAny(persons, person->person.getPid()>1);
	...
	public static <T> T firstMatchingWithAny(List<T> data, Predicate<T> matchFunction) {
		for (T emp : data) {
			if (matchFunction.test(emp)) {
				return emp;
			}
